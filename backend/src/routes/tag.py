...
from backend.src.util.schemas import tag as schemas_tag
...
@router.get("/tags/", response_model=list[schemas_tag.Tag])
def read_tags(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    tags = crud_tag.get_tags(db, skip=skip, limit=limit)
    return tags

@router.post("/tags/")
async def create_tag(tag: schemas_tag.TagCreate, db: Session = Depends(get_db)):
    print('create_tag')
