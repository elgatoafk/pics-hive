...
from backend.src.util.schemas import photo as schemas_photo, user, tag as schemas_tag
...
@router.post("/photos/", response_model=schemas_photo.Photo)
async def create_photo(
    ...
    tags_objects = [schemas_tag.TagCreate(name=tag) for tag in tag_list]
    
    # Create PhotoCreate schema instance
    photo_in = schemas_photo.PhotoCreate(
...
@router.get("/photos/{photo_id}", response_model=schemas_photo.Photo)
...
@router.put("/photos/{photo_id}", response_model=schemas_photo.Photo)
def update_photo(
    photo_id: int,
    photo: schemas_photo.PhotoCreate,
    db: Session = Depends(get_db),
...
@router.post("/photos/{photo_id}/transform", response_model=schemas_photo.Photo)
def transform_photo(
    photo_id: int,
    transformation: str,
    db: Session = Depends(get_db)
